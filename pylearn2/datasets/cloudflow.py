...
#            CLOUDFLOW.y_large[self.which_set] = np.zeros((num_examples, self.predict_dim), dtype='uint8')
        ...
        if os.path.isfile(X_path) and not self.show_mode:
            ...
            CLOUDFLOW.y_large[self.which_set][:self.example_cnt] = y
            del X
            del y
            print 'Done.'
        else:
            ...
            for i in range(self.train_frame_size[0] + self.predict_interval + self.predict_frame_size[0] - 1, 
                ...
                for _ in range(self.examples_per_image):
                    ...
                    if track_frames is None:
                        continue
#                    if track_frames[-1].sum() == 0.:
#                        continue
                    frames = track_frames
                    ds = cv2.resize(frames.transpose((1,2,0)), (0,0), fx=1./self.postdiv, 
                                    fy=1./self.postdiv, interpolation = cv2.INTER_AREA).transpose((2,0,1))
                    if ds[-1].sum() == 0.:
                        continue
                    x = ds.flatten()
#                    if x.sum() == 0.:
#                        print self.example_cnt, 'frames.sum() =', frames.sum(), \
#                        'frames[-1].sum() =', frames[-1].sum(), '(frames[-1]>0).sum() =', (frames[-1] > 0.).sum()
